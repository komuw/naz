...
class MockSmppServer:
    def __init__(
        self, client_connected_cb, port, host="127.0.0.1",
    ):
        """
        client_connected_cb: Is the callback is called whenever a new client connection is established.
                             It receives a (reader, writer) pair as two arguments, instances of the StreamReader and StreamWriter classes.
                             see: https://docs.python.org/3/library/asyncio-stream.html#asyncio.start_server
        """
    ...
    async def handle_echo(self, reader, writer):
        """
        this is an example implementation of `client_connected_cb`
        """
    ...
    def okay(self):
        ...
        self._thread = threading.Thread(
            target=self.haiya, name="Thread-<MockSmppServer>", daemon=True
        )
        self._thread.start()



class TestAla(TestCase):
    """
    run tests as:
        python -m unittest discover -v -s .
    run one testcase as:
        python -m unittest -v tests.test_client.TestAla.test_can_connect
    """
    ...
    def test_cool(self):
        ...
        mock_smpp_server = MockSmppServer(client_connected_cb=callback, host=host, port=port)
        mock_smpp_server.okay()

        client_ucs2 = naz.Client(
            smsc_host=host,
            smsc_port=port,
            system_id="smppclient1",
            password=os.getenv("password", "password"),
        ...
        # self._run(client_ucs2.connect())
        # self.assertTrue(hasattr(client_ucs2.reader, "read"))
        # self.assertTrue(hasattr(client_ucs2.writer, "write"))
        client_ucs2.current_session_state = "BOUND_TRX"
        self._run(client_ucs2.tranceiver_bind())
        mock_smpp_server.close()
